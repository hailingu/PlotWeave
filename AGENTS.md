...
| `.githooks/**`, `scripts/**`, and `sonar-project.properties` | This guide's Non-Negotiable Gates, Security, and Version-Control Safety sections | repository root | `npm test -- scripts/sonar-quality-gate.test.ts` | Versioned local Git hooks and the shared SonarQube incremental zero-issue gate (new code only). Install hooks with `npm run hooks:install`. |
