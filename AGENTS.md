...
| `src/**` and root frontend manifests or config (e.g., `package.json`, `tsconfig.json`, `vite.config.*`, `eslint.config.js`, `.prettierrc.json`, `.prettierignore`, `.nvmrc`) | This guide and `docs/development/software-engineering-standard.md` + `docs/development/typescript-standard.md` | repository root | `npm run format:check && npm run lint && npm run build && npm test` | React Flow（`@xyflow/react`）/ TypeScript frontend rendered inside the Tauri webview. npm is the package manager; the Node version is pinned by `.nvmrc` (24.18.0). Formatting is Prettier's responsibility (`npm run format` / `format:check`, configured by `.prettierrc.json` and `.prettierignore`); ESLint owns code quality rules, and the Rust backend stays with `cargo fmt`. |
