...
fn needs_no_quarantine(assets: &CapDir, index: &Value, rel: &str) -> Result<bool, LibraryError> {
    ...
    let suffix = rel
        .strip_prefix("assets/")
        .ok_or_else(|| LibraryError::invalid(format!("relPath 越出 assets/：{rel}")))?;
...
fn capture_original(
    assets: &CapDir,
    rel: &str,
) -> Result<(CapDir, String, cap_std::fs::File, (u64, u64)), LibraryError> {
    use cap_std::fs::MetadataExt;
    let suffix = rel
        .strip_prefix("assets/")
        .ok_or_else(|| LibraryError::invalid(format!("relPath 越出 assets/：{rel}")))?;
    let (parent, last) = open_parent_dir(assets, suffix)?
        .ok_or_else(|| LibraryError::missing(format!("资产父目录缺失：{rel}")))?;
    let md = asset_stat(&parent, &last, rel).map_err(LibraryError::from)?;
    ...
    let file = parent
        .open(&last)
        .map_err(|e| LibraryError::io(format!("打开待删资产失败（{rel}）"), e))?;
    let fm = file
        .metadata()
        .map_err(|e| LibraryError::io("读取待删资产句柄元数据失败", e))?;
...
#[cfg(unix)]
fn check_same_fs(trash: &CapDir, identity: (u64, u64)) -> Result<(), LibraryError> {
    use cap_std::fs::MetadataExt;
    let tm = trash
        .dir_metadata()
        .map_err(|e| LibraryError::io("读取隔离目录句柄元数据失败", e))?;
    if tm.dev() != identity.0 {
        return Err(LibraryError::refused(
            "隔离区与媒体不在同一文件系统，无法原子隔离",
        ));
    }
    Ok(())
}
...
fn commit_quarantined_delete(
    ...
    parent
        ...
        .map_err(|e| LibraryError::io(format!("隔离媒体失败（{rel}）"), e))?;
...
#[cfg(unix)]
fn quarantine_conflict_error(entry: &JournalEntry, verdict: &TrashVerdict) -> LibraryError {
    match verdict {
        TrashVerdict::Missing => LibraryError::missing(format!(
            "资产 {} 隔离项缺失，事务保留待恢复",
            entry.asset_id
        )),
        TrashVerdict::Mismatch => LibraryError::refused(format!(
            "资产 {} 隔离期身份不符：隔离项被替换，事务保留待恢复",
            entry.asset_id
        )),
        TrashVerdict::IdentityOk(_) => {
            LibraryError::refused(format!("资产 {} 隔离期身份冲突", entry.asset_id))
        }
    }
}
