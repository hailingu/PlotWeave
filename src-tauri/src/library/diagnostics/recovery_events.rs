//! 恢复观察的共享边界：锁内收集诊断，锁外通过同一序号域发布事件。
//! 命令或媒体/导入失败也不丢掉已完成恢复的警告；不延长媒体字节读取的锁。
...
pub(crate) fn with_recovery_snapshot<T, E: From<LibraryError>>(
    ...
    with_observations(
        library,
        |dir, report, _revision| operation(dir, report),
        |_, snapshot| publish(snapshot),
    )
}

/// 命令与替代入口共享锁、序号和显式恢复观察；发布策略由各自的响应契约
/// 决定，且一律在释放锁之后执行。恢复前拒绝或恢复失败不会产生观察。
pub(super) fn with_observations<T, E: From<LibraryError>>(
    library: &Dir,
    operation: impl FnOnce(&Dir, &mut dyn FnMut(&Recovery), u64) -> Result<T, E>,
    publish: impl FnOnce(&Result<T, E>, RecoverySnapshot),
) -> Result<T, E> {
    let (result, snapshot) = {
