...
| F6 发现、结构与配对 | glob 自动发现全部 src CSS，布局/动画/空表不设条数门槛；展示色及可达局部别名字面色禁用；例外绑定表/选择器/属性/值/条数并反向校验；8 个配对环境；危险底色颜色/图像成分各自层叠 | 非注册字面色、过期或扩大的例外；配对阈值不达标；危险前景或背景有效接线变化 | box-shadow 层级投影不在展示色禁用范围；#240 恒白危险前景及深色约 2.8:1 和非文本对比度观察项沿用既有处置（#262 品牌配对与 #265 悬空引用均已修复移出例外表：两处品牌底消费点统一 on-brand × edge-label-bg 配对，--fill-tertiary 四环境令牌定义）；品牌底白字在浅深默认外观（no-preference）均实测低于 WCAG 2.2 普通文本 4.5:1 参考（青端 2.488:1/1.765:1），次级文本仅浅色默认外观低于参考（3.261:1/2.968:1，深色默认 `#8e8e96`/`#131316` ≈ 5.70:1 达标），issue #351 端点算术登记，F6 配对门禁不含上述 no-preference 路径，more 下的通过不推广到默认主题；不新增产品配色决策 |
...
已知边界处置：[#290](https://github.com/hailingu/PlotWeave/issues/290)（带 fallback 的令牌在配色检查中被误拒绝）已修复——`resolveChain` 支持主值/fallback 选择（深度保护不变），配对与危险底色检查路径的 fallback 形态由引用测试与配对助手测试覆盖；[#289](https://github.com/hailingu/PlotWeave/issues/289)（非 ASCII 自定义属性名末端被误当空白裁剪）已修复——各 `var()` 求值入口统一按 CSS 空白集（空格/制表/换行/回车/换页）裁剪参数，不再使用 JS `.trim()`，名称按原始码点序列匹配（含末端 NBSP 等有效码点），由同名消解、异名悬空与 fallback 组合测试覆盖；[#349](https://github.com/hailingu/PlotWeave/issues/349)（大量同级 `var` 引用可使求值器栈溢出）已修复——`resolveDisplayValue` 的同级余串由递归改为一次掩码 + 游标线性推进（栈深恒定），同级不递增链深的 #290 语义与依赖/fallback 32 层预算保留，大同级列表与深依赖两个维度由引用回归测试覆盖；[#339](https://github.com/hailingu/PlotWeave/issues/339)（NBSP 包围的取值被误判为 CSS-wide 关键字）已修复——保证无效判定（`isGuaranteedInvalid`）与展示色类型检查入口（`colorTypeOk`）的关键字/空值分类统一改用 CSS 空白集裁剪（`trimCssWhitespace` 下沉至 `cssValueSyntax` 共享，求值与类型入口不分叉），简写层/颜色列表的顶层切分（`splitCssSpace`/`splitCssComma`）同样按 CSS 空白集裁剪成分并补齐回车/换页分隔符（`postcss.list` 的成分裁剪用 JS `.trim()`，会把 NBSP 包围的标识符折算回关键字或完整颜色成分），图像长形的逗号切分（`imageValueOk`）入列裁剪同款，`\u00a0unset` 按普通标识符原样消解、颜色语义照常拒绝非法值，类型诊断按同一空白语义呈现值，由 `resolveChain` 与共享显示值路径的 NBSP 回归（含普通空白/大小写对照）覆盖。当前未解决的已登记边界仅 [issue #351](https://github.com/hailingu/PlotWeave/issues/351)（登记不修的基线视觉边界），按承载面分两类，均低于 WCAG 2.2 [普通文本 4.5:1 参考](https://www.w3.org/WAI/WCAG22/Understanding/contrast-minimum.html)：**品牌渐变端点白字**在浅深两外观默认（no-preference）下均低于——白字/浅色青端 `#00b3d8` 2.488:1、白字/深色青端 `#21d4fd` 1.765:1（补充品红端：浅 `#e0176e` 4.649:1 达标、深 `#ff2e88` 3.501:1 低于）；**text-secondary** 仅浅色默认外观低于——`#8e8e93`/白 3.261:1、`#8e8e93`/窗口底 `#f4f4f6` 2.968:1，深色默认 `#8e8e96`/`#131316` 实测 ≈5.70:1 达标、不属本边界（标准 sRGB 相对亮度公式的端点算术，消费点含分支边胶囊、AI 活跃按钮与用户消息的 on-brand × edge-label-bg 配对）。已关闭的 [#262](https://github.com/hailingu/PlotWeave/issues/262) 明确保留基线视觉，本边界不是其修复失败：不把 more-only 配对门禁推广到默认主题、不新增产品配色决策，未逐字形/逐半透明承载面采样，不能据端点宣称每个控件所有文字都失败；是否调整基线视觉/阈值由所有者排期（若改进，须覆盖普通小字号真实消费点、完整渐变范围与半透明合成，浅/深及 more，不能只测一个端点）。用户指定内容配色（on-saturated）与装饰图案不在对比度契约内的原有范围区别不变。数值与范围同步登记于 UI 设计 §2.1、§2.2、§2.3、§2.6。
...
| F6-b | 浅深、more、降透明度及 hover 配对 | 固定令牌承载面对比度与黄金接线 | primary ≥4.5，secondary more ≥4.5；错误横幅 fg/bg 四环境 ≥4.5 且接线绑定消费点（换任一声明接线即失败，issue #318）；危险恒白按 #240 | 配对与产品已接受决策一致；配对/危险契约 | 8 环境既有测试；非文本对比度仍按既有处置（#262/#265 已修复移出例外表，品牌底消费点配对由 sheetTokens BRAND_RULES 黄金接线与 more 配对采样持有）；no-preference 下品牌底白字与次级文本配对均不在门禁内，低于参考的仅品牌底白字（浅深）与浅色次级（issue #351 登记的基线视觉边界，不据此新增默认主题门禁） |
