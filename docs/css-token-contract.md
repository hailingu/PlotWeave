...
未解决边界登记：[#289](https://github.com/hailingu/PlotWeave/issues/289)（非 ASCII 自定义属性名末端被误当空白裁剪，F5-c 词法边界）为 open 状态的已知边界；修复落地前，相关入口的保留范围以上述边界表与该 issue 的登记为准——其使 F5 行「未转义非 ASCII 自定义属性名完整保留」及矩阵 F5-c 相应条目受限。[#290](https://github.com/hailingu/PlotWeave/issues/290)（带 fallback 的令牌在配色检查中被误拒绝）已修复：`resolveChain` 支持主值/fallback 选择（深度保护不变），配对与危险底色检查路径的 fallback 形态由引用测试与配对助手测试覆盖。
