...
| F6-b | 浅深、more、降透明度及 hover 配对 | 固定令牌承载面对比度与黄金接线 | primary ≥4.5，secondary more ≥4.5；错误横幅 fg/bg 四环境 ≥4.5（issue #318）；危险恒白按 #240 | 配对与产品已接受决策一致；配对/危险契约 | 8 环境既有测试；非文本对比度仍按既有处置（#262/#265 已修复移出例外表，品牌底消费点配对由 sheetTokens BRAND_RULES 黄金接线与 more 配对采样持有） |
