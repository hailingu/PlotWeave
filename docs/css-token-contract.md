...
未解决边界登记：[#289](https://github.com/hailingu/PlotWeave/issues/289)（非 ASCII 自定义属性名末端被误当空白裁剪，F5-c 词法边界）与 [#290](https://github.com/hailingu/PlotWeave/issues/290)（带 fallback 的令牌在配色检查中被误拒绝，F5-b 求值边界）为 open 状态的已知边界；修复落地前，相关入口的保留范围以上述边界表与两条 issue 的登记为准。
...
| [5286812629](https://github.com/hailingu/PlotWeave/pull/288#pullrequestreview-5286812629) | F2-c 颜色成分识别统一 `completeColorAtom`：transparent/具名色/currentColor 经颜色长形覆盖后不冒充 danger（针对 `0516b47` 的后续修订修复） |
...
| [5287668653](https://github.com/hailingu/PlotWeave/pull/288#pullrequestreview-5287668653) | F5-c 未转义非 ASCII 自定义属性名词法；F4-c 遮罩 RGB 函数名大小写（修复 `8456bdb` 之后的后续修订） |
