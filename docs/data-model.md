...
├── recovery/                          # 会话恢复副本（主文件保存失败时的跨进程保留）
│   └── ai-session-{projectId}.json    # 权威会话落盘成功后即清除
...
**AI 会话存储（issue #47，已实现）**：`ai-session.json` 的 `schemaVersion: 1` 保存消息 id、角色、顺序、正文与预览卡/执行回执状态；它只记录历史，不恢复撤销栈，也绝不自动重放命令。读取旧项目或缺文件返回空历史；前端逐条归一化，局部损坏条目隔离并回写可修复部分，完整文件损坏则仅报告会话恢复失败，不阻断 `project.json`。同一项目的会话保存串行化并用同目录临时文件、flush、原子 rename 与父目录同步提交；失败必须上浮给面板，内存历史保留，后续变更可重试。**恢复副本（历轮评审修复）**：主文件保存失败后，前端把同一会话尽力写入与 `projects/` 同级的 `recovery/ai-session-{projectId}.json`（写入前仍要求项目记录存在，已删除项目不得留下不可见聊天数据）——项目目录不可写或记录暂不可读时它仍可能写入成功，使内存中的唯一拷贝跨进程存活；读取会话时主文件与副本各自记录**写入序号**（单调计数，跨进程从磁盘两副本的最大值续起、并与内存已推进的序号取大合并——乱序返回的并发载入不得回拨，否则下一次保存与权威文件同号，主保存失败时副本以相同序号落盘而在重开时不被选中，新历史丢失；与墙上时钟无关，时钟回拨或同毫秒写入都不会错序），取序号较大者（副本只在主文件失败后写入，序号必然更大；但权威保存成功而副本清除/改写失败时副本序号更小，较新的权威会话必须胜出），载入后（主文件可读时）立即尝试提升为权威副本，提升失败则继续保留副本并在界面标明「已从恢复副本载入」（主文件不可读时只展示恢复历史，不发起任何写回——序号未知，写回失败路径的副本改写与重试登记会让陈旧历史在主文件恢复可读后覆盖更新的权威会话）；权威保存成功或删除项目时清除副本（清除失败则改写为刚落盘的权威内容，陈旧副本不得在下次载入时把新会话回退成旧内容；改写失败只记录，不使已成功的保存失败）。写入边界带**顺序守卫**：现存主文件与恢复副本都受同一约束——不可读（权限/瞬态 I/O）、写入序号更新，或**相等序号但对话内容不同**（无单实例约束下两进程各自从同一磁盘序号续起撞号的并发冲突——后写替换先写会把一侧会话静默丢失且写入边界还报告成功；对话内容一致的相等序号写入按同一逻辑写入的幂等重放放行，含旧无序号文件补显式 writeSeq 0 的升级）时整次拒绝，且守卫的检查、权威落盘与副本清理全程持有 `recovery/session-write.lock`（永不被替换的稳定锁文件）上的跨进程排他文件锁（评审 pullrequestreview-5161978174：守卫是「检查后写入」，不锁则两进程可同时通过旧序号检查、再先后原子改名，后者静默替换前者；锁随进程退出由内核释放、无陈锁，读取不经锁——原子 rename 保证读到某个一致的副本快照），载荷携带的 writeSeq 存在时必须是**非负安全整数**（超出 JS 安全整数范围的序号一旦落盘，前端续号会被守卫当作更旧而永久拒绝；已落盘的超范围序号文件按损坏归类，可被合法保存替换），不写主文件也不清除副本（内容可读但不可解析的损坏文件无法再被载入，仍可安全覆盖修复）——绝不在无法确定新旧时销毁可能是更新权威副本的历史。前端另在保存入口设**新旧未知门禁**：载入发现任一副本不可读（真实 I/O 失败，即两条 load 命令的 Err）即登记该项目，此后一切会话保存（含用户变更、挂载重试与退出冲刷）整次暂缓——不写盘、不定序、不登记重试，避免序号在未知基线上自增并最终压过恢复可读的更新副本；两副本可读的重新载入即解除（保留区会话的重开路径同样重新读盘）。**损坏不进门禁**（评审 pullrequestreview-5161801056/5161978174）：主文件与恢复副本的不可解析或信封非法内容均无法被任何进程载入，两条 load 命令以 `{ session: null, corrupt: true }` 返回（共用 SessionCopy 契约，与顺序守卫的可替换归类同口径）——主文件损坏时可用恢复副本胜出并立即提升修复（写入边界按可替换覆盖主文件），两者都损坏或无可用历史时按空历史打开并提示，均在下次保存成功时自愈，后续保存正常落盘，无须手工清理磁盘。暂缓的编辑**登记进退出屏障**（评审 pullrequestreview-5161801056）：它只存在于内存，不得写盘但退出必须知情——窗口关闭/⌘Q 被阻止并给出「退出将丢弃这些编辑」的专门诊断，重开项目确立定序或删除项目时清出登记。前端对应两种不可读：恢复副本读取失败时展示权威历史，权威文件读取失败时展示恢复副本；两者都标明「已暂缓保存以免覆盖更新的历史」并禁止挂载自动重试，且不发起提升覆盖。副本写入同样失败时仍上浮原始保存错误，并把该会话登记进**进程内重试**（固定节律重试、旧代次定时器一律替换，存储恢复即自愈；重试补写成功时通知 UI 层清除项目级错误与保留快照——后台重试不经面板保存通道，只有该通知能撤下「保存失败」横幅；保存被拒后先重读两副本序号做**冲突检测**：磁盘序号已不小于本次写入（无单实例约束下另一进程已写更新历史）即视为陈旧快照，不写副本、不登记重试并清出既有登记——逐次自增的重试终将压过磁盘序号、以旧历史覆盖新会话，绕过顺序守卫；上浮原错误让用户重开项目载入更新的历史），内存副本继续保留；窗口关闭时若仍有**排队/在途**的保存，存在主文件与副本都未写入的会话（唯一副本只在内存），或存在新旧未知期间被整次暂缓的编辑（同样只在内存，冲刷不重试写盘、只作为阻断项上浮），前端阻止关闭并把待保存集合排空到**固定点**——先等在途落定、再冲刷全部待重试会话，冲刷与销毁的间隙新增的保存（拦截后的窗口仍可交互，同项目的后来者会替换在途登记而不加入先前捕获的快照）经重查后再排空一轮，仍有不可恢复项则保留窗口并给出可见诊断——绝不让唯一内存副本随进程消失；副本已落盘的待重试会话不阻止退出（跨进程可恢复）（macOS ⌘Q 经 Rust 侧接管应用菜单后与窗口关闭共用同一道冲刷屏障：tao 的原生 terminate 不可拦截，而 ⌘Q 是菜单加速键——自定义「退出」菜单项只发出 app-quit-requested 事件，前端排空后走受控 app_exit，仍有不可恢复项则不退出并显示诊断；系统级强制终止（kill 等）仍不可拦截，属既有文档边界）；磁盘整体不可写且进程被强制结束时仍只剩内存拷贝，该边界无任何写入通道可用。删除项目删除整个项目目录，因此同时删除会话与恢复副本——副本清除失败时删除本身仍已提交，命令返回「已提交 + 清理诊断」回执；恢复目录在删除提交后才打开，它暂不可用（不可读/被非目录条目占位）同样不阻断任何项目的删除，只并入该清理诊断：前端不得把该回执当作删除失败回吐在途/吸收的画布保存（`save_project` 会重建已删项目记录，项目在用户删除后复活而资产目录已移除），残留副本由 `list_projects` 在启动/每次刷新时清扫——「恢复副本存在但对应项目记录已缺失」即孤儿（记录是权威存在性来源，缺失即副本永不可达），跨进程兜底不留不可发现的聊天数据。复制项目仅复制画布和项目资产，**不**复制原项目对话；现有剧本导出只导出画布内容，也不包含会话。

### 10.2 写入安全

单写者场景**不需要**文件锁、版本号等并发机制；但需要防崩溃截断——写到一半进程被杀、断电、磁盘满，会留下截断的 JSON，导致整个项目无法打开。因此：

- **控制文件信任链是所有 I/O 的前置条件**：Rust 先 canonicalize `app_data_dir` 为受信根；`projectId` 等路径分量先通过对应的文件名安全字符集校验，再参与拼接。对 `projects/`、`library/` 及目录化布局的 `projects/{projectId}/`，现存目录必须是实际目录且 canonical 路径逐级仍位于父级受信根内，任一级为符号链接（无论指向根内或根外）、非目录或无法验证都拒绝**整个对应操作**，不能只禁用其 `assets/`。缺失目录只能在已验证父目录下创建，创建后立即 canonicalize 复核。读取、创建、替换、删除或扫描 `project.json`、旧布局 `{projectId}.json`、`index.json`、`recovery/ai-session-{projectId}.json`、`library/library.json`、`library/asset-delete-journal.json`、`settings.json` 及其临时文件前，使用 `symlink_metadata` 拒绝最终路径为符号链接，现存文件还须是普通文件且 canonical 后仍是已验证父目录的直接后代；目录扫描跳过并报告符号链接/越界项，绝不跟随。验证后保持父目录句柄打开，所有实际 open/create/rename/delete 均以平台等价的 no-follow、相对目录句柄语义执行，不得 canonicalize 后退回未经绑定的原字符串路径，避免检查与使用之间被替换。新临时文件以该目录句柄下的随机同目录名称排他创建，避免预置 `.tmp` 符号链接截获写入；最终 rename 前再次验证现存目标。该解析器由项目 list/create/load/save/delete/复制流程、布局迁移、索引校正及库/设置命令共用，验证失败时不得读取、写入、删除外部树中的文件，也不得先更新另一控制文件。
...
| `load_ai_session(projectId)` / `save_ai_session(projectId, session)` | 当前已实现：经项目目录受信句柄读取/原子写 `ai-session.json`；缺文件返回空历史（`session: null`），JSON 或信封损坏以 `{ session, corrupt }` 按可安全替换归类返回而不影响画布加载（真实 I/O 失败才 Err，见 §10.1）。保存只接受会话 v1 信封与数组 entries；逐条恢复归一化在前端会话模型完成。权威保存成功后清除同名恢复副本；守卫检查、落盘与副本清理全程持有跨进程写入锁（§10.1）。 |
| `stash_ai_session_recovery(projectId, session)` / `load_ai_session_recovery(projectId)` | 当前已实现（历轮评审修复）：主文件保存失败后的跨进程保留——经受信根句柄原子写/读 `recovery/ai-session-{projectId}.json`（§10.1），写入前仍要求项目记录存在，副本顺序守卫同样在跨进程写入锁内执行。读取返回与 `load_ai_session` 共用的 `{ session, corrupt }`（SessionCopy）契约：`session` 为 null 表示副本缺失或损坏（损坏时 `corrupt` 置位——按可安全替换归类，前端不得据此进入新旧未知门禁），Err 仅保留给真实 I/O 失败。副本写入同样失败时前端保留内存副本并上浮原始保存错误。 |
...
- **会话生命周期（issue #47）**：会话状态属于项目而非可卸载的 AI 面板；切换检查器、折叠右栏时不重置。历史恢复只展示内容与回执，`executed` 预览卡不会再次执行，且因撤销栈不随会话持久化，恢复的历史执行卡呈现为「历史改动」、不宣称 ⌘Z 可整批撤销（执行回执同样只记录事实）；`pending` 卡即使由历史显示，执行入口仍按当前画布重校验并再次要求用户确认。完整历史仅用于界面展示，喂给模型的历史按条数与累计字符双界自旧侧截断，持久化会话的跨重启增长不会顶穿供应商上下文上限。会话保存失败上浮面板并写入项目级错误：警告跨编辑器重挂载保留，重挂载期保存在途、拒绝晚到时项目级错误同步进已挂载的新面板，重挂载或后续会话变更都会重试落盘，保存成功即清除该提示（含后台重试补写成功经通知清除——面板错误随项目级状态双向同步）；但重试只针对内存中为恢复会话或最新会话的失败——读取失败（`load_ai_session` 拒绝）时内存是空回退会话，禁止作为挂载重试的落盘内容（原子覆盖会毁掉可能可恢复的原文件），须等用户实际变更对话——变更被接受即视为权威用户内容并恢复可重试标记——后才落盘（**新旧未知门禁**：任一副本不可读期间，变更保存同样被保存入口整次暂缓——不写盘、不定序、不登记重试，否则失败路径的副本改写与重试自增会把序号压过恢复可读的更新副本；编辑保留在会话与保留区，重开项目重新读取两副本确立定序后恢复落盘——保留会话的重开路径也重新读盘刷新门禁，否则保留重试被永远暂缓；重开时保留快照与读得的磁盘历史**调和**（评审 pullrequestreview-5161978174）：磁盘历史完全包含于保留快照（逐条前缀）时保留快照胜出重试（覆盖无丢失），分叉或磁盘更长时载入磁盘版本并如实告知被暂缓的编辑未能与之合并——绝不以保留快照带更高序号直接覆盖未调和的历史；暂缓的编辑同时登记进退出屏障，窗口关闭/⌘Q 被阻止并告知退出将丢弃，不随进程静默消失；损坏（主文件或副本）不进门禁，见 §10.1）。保存失败后的未落盘会话保留在打开项目视图之外：返回首页不丢弃内存副本，重开项目时以保留会话恢复、呈现保留提示并自动重试落盘，成功后清出保留区；删除项目同时清出。保留区在项目加载屏障（共享保存链落定）之后读取——保存在途时重开项目，拒绝处理器在屏障等待期间写入保留区，屏障前先取会漏掉最新副本。恢复条目的 id 一律重定基为 1..n 有界序列后继续自增：落盘 id 不受信，防止自增越过 `Number.MAX_SAFE_INTEGER` 产生重复渲染 key 与下次加载被归一化丢弃的条目。正在进行的网络请求不在重启后恢复为 busy 或自动重发；项目切换/关闭后的迟到结果不写入新项目。执行回执的落盘与画布持久化以**提交身份**解耦：画布文档持有 `graph.aiRevision`（已应用 AI 批次的单调计数，缺省 0；只增不减，撤销不回退），每次成功落地一批命令自增一次。批次在内存执行后，卡片先以 `pending`（附执行后计数 `aiRevisionAfter`）落盘并按回执与卡片的关联（回执追加在会话尾部，不能按位置识别）压掉该卡的回执，承载批次的画布文档确认落盘（`whenCanvasCommitted`：注册之后开始的保存成功落定，在途的旧保存不算数）后才写 `executed` 与回执——画布未持久化时重开得到可重新执行的待执行卡，而不是声称已执行的历史卡。重开时带执行后计数的待执行卡与画布内计数对账：画布计数已达该值 = 批次已随画布落盘（涵盖执行后立即离开编辑器、卸载冲刷或项目级后台重试补落盘等路径），恢复为历史执行卡；未达 = 未落盘，按当前画布重校验后仍可再次执行。计数是文档内的持久身份，普通画布编辑与未落盘编辑都不影响判定——不以「与执行前签名不同」这类推断代替。会话保存失败后的跨进程保留见 §10.1 的 `recovery/` 恢复副本：主文件与副本各记录**写入序号**（单调计数、与时钟无关），载入取序号较大者（权威保存成功但副本清除/改写失败时陈旧副本不得回退新会话），副本胜出后自动提升为权威副本（主文件可读时；不可读时只展示恢复历史，不发起写回），提升失败时界面标明来源、继续保留副本。
