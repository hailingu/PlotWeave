...
- **自由文本写入体积预算（[issue #170](https://github.com/hailingu/PlotWeave/issues/170)，已实现）**：AI 可写自由文本按字段分级设上限——普通自由文本字段（节点 name/synopsis/picture/prompt/tone/time/weather/size、lines[].text、options[].label、refs[].label 自由文案、实体 name/bio/note、文档 title）≤ 65,536 字符；设定文档 body 因「长篇正文」定位放宽至 ≤ 1,048,576 字符（对齐文件级 1 MiB 先例）。预算常量在 `ai/textBudget.ts` 单点维护：入站校验（patchShape/entityFold）按域整批拒绝并回喂点名字段与实际字数的诊断，**不自行截断用户内容**；工具 schema 经协议表 `maxLength` 向模型广告同一预算（广告是输出协议，校验边界仍是权威执行点）。单批总体积不另设预算——模型工具调用参数随对话响应体受 §10.3 的 16 MiB 上限兜底；本预算管单字段/单成员的病态极值（审计探针曾实测 2 MiB synopsis 被原样接受）。上限内既有内容（含旧项目中超预算的既有文本）不受影响——预算只约束 AI 写入边界，加载归一化不以此拒绝或截断。
