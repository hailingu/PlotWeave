...
> 八十八轮修订（2026-09-19，[issue #135](https://github.com/hailingu/PlotWeave/issues/135)）：§7.2 `cleanupPending` 的呈现由仅控制台改为经既有恢复诊断通道对用户可见（ui-design §8.1 异常反馈）——按快照显示待清理计数、影响（磁盘空间未释放，不误报为已回收）与恢复指引；内容随各操作返回整体替换，空状态不显示。身份校验、fail-closed 与按日志恢复的既有行为不变，不新增自动物理删除。
