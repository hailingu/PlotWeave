...
**relPath 安全约束**：relPath 必须是纯相对路径（禁止绝对路径），规范化后不得越出其根目录——含 `..` 上跳段的解析目标越出项目/库资产根即非法。这是 §7.1「项目自包含」与备份/移动完整性的前提，也是信任边界校验（防解析或复制时逃逸资产根）：`set_asset` 命令边界拒绝非法值，归一化对脏数据隔离该索引项并记录警告（引用该资产的字段按 §8.2.3 悬空展示）。
