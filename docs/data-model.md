...
| `stash_ai_session_recovery(projectId, session)` / `load_ai_session_recovery(projectId)` | 当前已实现（历轮评审修复）：主文件保存失败后的跨进程保留——经受信根句柄原子写/读 `recovery/ai-session-{projectId}.json`（§10.1），写入前仍要求项目记录存在；读取缺失返回 null。副本写入同样失败时前端保留内存副本并上浮原始保存错误。 |
