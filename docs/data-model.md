...
`graph`/`settings`/`assets` 三个容器可携带上表契约键之外的**同版本扩展字段**（未知键，[issue #100](https://github.com/hailingu/PlotWeave/issues/100)）：Rust 原样透传、前端归一化按扩展字段保留（不修复、不回写），保存原样落盘——唯一例外是超出 JS 安全整数域的整数：跨 IPC 边界解析时即舍入，归一化记诊断警告、保存固化当前加载值（§11 分层策略与传输边界）。顶层与 `project` 层是类型化封闭契约，未知键不保留；会被旧客户端丢弃的字段增补必须升级 `schemaVersion`。
...
- **`graph`/`settings`/`assets` 容器层（扩展字段保留）**：三个容器以 untyped 值经 Rust 透传（第 0 步），契约键（graph 的 `nodes`/`edges`/`viewport`/`aiRevision`，settings 的四桶，assets 的 `byId`）之外的未知键由前端归一化按扩展字段原样保留：解析不修复、不纳入 `repaired` 判定（仅打开不回写），经会话透传字段随下一次保存原样落盘；保存边界（§10.5）只校验契约字段，扩展键照常通过。**传输边界**：扩展值跨 Rust→webview 的 JSON IPC，超出 JS 安全整数域（±2^53−1）的整数在 webview 解析时即舍入——先于前端一切代码，不可检测、不可修复；归一化对该类值记域约束诊断警告（打开仍零回写），下一次保存固化当前加载值，需要位精确大整数的字段演进必须升级 `schemaVersion` 走类型化契约。实现在 `normalizeContainers`（捕获 + 值域诊断 + 装配回填）与 `serialize.ts`（会话 ⇄ 落盘双向携带），契约键清单共享于 `jsonGuards.ts`。
