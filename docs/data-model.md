...
| `load_ai_session(projectId)` / `save_ai_session(projectId, session)` | 当前已实现：经项目目录受信句柄读取/原子写 `ai-session.json`；缺文件返回空会话，JSON 或信封损坏显式报错而不影响画布加载。保存只接受会话 v1 信封与数组 entries；逐条恢复归一化在前端会话模型完成。权威保存成功后清除同名恢复副本。 |
| `stash_ai_session_recovery(projectId, session)` / `load_ai_session_recovery(projectId)` | 当前已实现（历轮评审修复）：主文件保存失败后的跨进程保留——经受信根句柄原子写/读 `recovery/ai-session-{projectId}.json`（§10.1），写入前仍要求项目记录存在。读取返回 `{ session, corrupt }`：`session` 为 null 表示副本缺失或损坏（损坏时 `corrupt` 置位——不可解析/信封非法的内容无法被任何进程载入，按可安全替换归类，前端不得据此进入新旧未知门禁），Err 仅保留给真实 I/O 失败（权限/瞬态 I/O，新旧未知）。副本写入同样失败时前端保留内存副本并上浮原始保存错误。 |
