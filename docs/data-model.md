...
**AI 会话存储（issue #47，已实现）**：`ai-session.json` 的 `schemaVersion: 1` 保存消息 id、角色、顺序、正文与预览卡/执行回执状态；它只记录历史，不恢复撤销栈，也绝不自动重放命令。读取旧项目或缺文件返回空历史；前端逐条归一化，局部损坏条目隔离并回写可修复部分，完整文件损坏则仅报告会话恢复失败，不阻断 `project.json`。同一项目的会话保存串行化并用同目录临时文件、flush、原子 rename 与父目录同步提交；失败必须上浮给面板，内存历史保留，后续变更可重试。**恢复副本（历轮评审修复）**：主文件保存失败后，前端把同一会话尽力写入与 `projects/` 同级的 `recovery/ai-session-{projectId}.json`（写入前仍要求项目记录存在，已删除项目不得留下不可见聊天数据）——项目目录不可写或记录暂不可读时它仍可能写入成功，使内存中的唯一拷贝跨进程存活；读取会话时主文件与副本各自记录写入时刻，取较新者（副本只在主文件失败后写入，正常情况下更新；但权威保存成功而副本清除/改写失败时副本是陈旧的，较新的权威会话必须胜出），载入后立即尝试提升为权威副本，提升失败则继续保留副本并在界面标明「已从恢复副本载入」；权威保存成功或删除项目时清除副本（清除失败则改写为刚落盘的权威内容，陈旧副本不得在下次载入时把新会话回退成旧内容；改写失败只记录，不使已成功的保存失败）。副本写入同样失败时仍上浮原始保存错误，内存副本继续待重试——该情形只剩内存拷贝（磁盘整体不可写的极端场景，无任何写入通道可用）。删除项目删除整个项目目录，因此同时删除会话与恢复副本；复制项目仅复制画布和项目资产，**不**复制原项目对话；现有剧本导出只导出画布内容，也不包含会话。
