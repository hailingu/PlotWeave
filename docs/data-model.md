...
| `load_project(projectId)` | 读 `project.json`；信封级兼容（旧扁平格式包装为 v0 信封），并随原始文档返回受信 `projectId` 与可用的索引元数据。v1 的 `project` 父容器或成员异型不得在 Rust 层整份拒绝，交由前端归一化修复；节点级 schemaVersion 迁移与归一化同样在前端模型层（见十一），Rust 不参与 |
...
`load_project` 返回后、交付画布前执行归一化管线，保证任何历史版本的文档都以当前形态进入会话。管线位于**前端模型层**（纯 TS，无框架依赖，与 §2 分层一致）——Rust 持久化层对节点/边结构不透明，只做信封透传、旧格式包装，并提供路径给定的受信 `projectId` 与索引元数据；v1 文档的 `project` 父容器或成员异型不得在 Rust 层整份拒绝。信封级兼容（旧扁平格式缺 `schemaVersion` 时包装为 v0 信封）由 Rust 在 `load_project` 内完成。唯一例外是资产 relPath 的真实路径包含判定（canonicalize/符号链接解析）——只能由可访问文件系统的 Rust 层在 `load_project` 内执行，非法条目清单随加载结果返回，供第 3 步消费（§7.1 分层执行）。
