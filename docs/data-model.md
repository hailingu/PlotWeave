...
interface AssetRef {
  id: string
  relPath: string            // 相对各自的资产根：项目资产相对项目 assets/ 子目录，库资产相对 library/assets/ 子目录——资产根是专用子目录，project.json/library.json/index.json 等控制文件位于根外、relPath 永不可解析到它们。安全约束见下
  mime: string
...
**relPath 安全约束**：relPath 必须是纯相对路径（禁止绝对路径），规范化后不得越出其根目录——含 `..` 上跳段的解析目标越出资产根即非法。**资产根 = 专用子目录**（项目 `assets/`、库 `library/assets/`，§10.1），而非整个项目/库目录：控制文件（`project.json`/`library.json`/`index.json`）位于根外，即使词法与真实路径校验全部通过，`delete_library_asset`/`remove_file` 之类的按路径操作也永远触达不到索引自身——以整个项目/库目录为根时，`"library.json"` 这样的条目词法合法、canonicalize 后仍在根内，却会让删除资产变成删除索引。词法规范化不足以兜底：资产目录内若存在指向根外的符号链接，`assets/link` 词法合法、不含 `..`，真实路径却已越界——因此校验以**真实路径包含关系**为准：读取/解析资产时对资产根（专用子目录）与目标路径做 canonicalize（解析符号链接）后判定目标仍位于资产根之内，越界即非法；新增资产（目标文件尚不存在、无法 canonicalize）时拒绝路径各级中的符号链接，或对最近已存在祖先做 canonicalize 后拼接校验。这是 §7.1「项目自包含」与备份/移动完整性的前提，也是信任边界校验（防解析或复制时逃逸资产根）：`set_asset` 命令边界拒绝非法值，归一化对脏数据隔离该索引项并记录警告（引用该资产的字段按 §8.2.3 悬空展示）。**库索引入口同款约束**：`library.json` 不走 §11.1 项目文档归一化（它不是 ProjectDocument），且可被手工修改或损坏——读取库索引（`list_library_assets`，§7.2 启动时全量载入）时对每个条目应用同款校验（含真实路径包含判定），非法条目不进内存索引并记录警告。仅在列表入口过滤不足以兜底：`delete_library_asset` 等操作会**重读磁盘索引**、按其中的 relPath 直接调 `remove_file`，而索引可能在列表校验之后、删除之前被替换（TOCTOU）——因此凡按 relPath 触达文件系统的入口（读取、mediaUrl 拼接、删除）都在**操作当时**重新执行真实路径包含校验，校验通过才放行；或者命令只消费本会话已通过校验的内存条目、绝不重读磁盘取路径。二者必居其一，项目侧按 relPath 的文件操作同论。由此库侧没有能触达越界路径的入口，而不只是约束新导入的资产。
...
interface CommandPayloads {
  ...
  // 会把既有同 id 边一并误删。剧情流边（sequence/branch）另须保持 DAG：
