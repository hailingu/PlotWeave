...
interface LabeledMeta {
  label: string
  episodeNo?: number             // 集归属（大纲分组的唯一依据）；正整数（Number.isInteger 且 > 0，与 set_episode_title/§11.1 归一化同域）
...
interface EdgeBase {
  ...
  source: string
  target: string
  targetHandle?: string                   // 必须省略：各节点仅一个匿名 target 端口（Handle 无 id），携带任意值即非法（§5 端口归属反向约束）
}

interface SequenceEdge extends EdgeBase {
  sourceHandle?: string                       // 必须省略：剧情流出口为匿名 output 端口（Handle 无 id），携带任意值即非法（§5 端口归属反向约束）
...
interface CommandPayloads {
  ...
  // 非 shot 节点的 episodeNo 须为正整数（Number.isInteger 且 > 0，与
  // set_episode_title 同域——零/负/小数/非有限值产生的大纲分组没有合法
