...
| `import_library_asset(file, meta)` | 按 §7.2 校验 meta 与完整构造结果后，按 §7.1 通过受信库资产根句柄安全拷贝入 `library/assets/`，再原子更新库索引；meta 含 name/kind/view/groupId/tags |
...
| `delete_library_asset(assetId)` | 从当前规范化索引解析 id 后，按 §7.1 通过受信库资产根句柄执行最终组件不跟随的相对删除，再原子删除索引项；不得把缓存 relPath 直接交给 `remove_file`（不影响已拷入项目的副本） |
| `collect_library_asset(projectId, projectAssetId, meta)` | 按 §7.1 分别以受信项目/库资产根句柄 no-follow 读取与写入，把项目资产拷贝入资产库（「收藏」） |
