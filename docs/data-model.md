...
- **历史展示**：不自动执行命令；已执行卡呈现为历史改动，不承诺当前撤销栈可撤销。待执行卡按当前画布重校验并再次确认。消息 id 恢复时重定基为 1..n；模型上下文按条数和累计字符裁剪，持久化条数另有保存时上限——自最旧起仅保留最新 200 条（[issue #64](https://github.com/hailingu/PlotWeave/issues/64)），加载不裁剪、旧文件全量展示，下次实际变更保存才收敛。重启不恢复网络请求 busy 或自动重发，项目切换后的迟到响应不写入新项目。
