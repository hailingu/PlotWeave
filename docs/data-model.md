...
> 八十六轮修订（2026-09-19，[issue #141](https://github.com/hailingu/PlotWeave/issues/141)）：§10.5 `llm_image_generate` 补作业总时间预算 600s（自命令进入起算）——url 回退下载链的逐跳 DNS 解析、请求与响应体读取的等待上界均为作业剩余预算，预算耗尽按阶段（解析图像主机/下载图像/读取图像）给出诊断并放弃，产物不写回；逐跳 120s 与生成 POST 300s 客户端上限保留为单阶段兜底。阻塞系统 DNS 任务无法取消，超时只是放弃等待，任务返回后结果被丢弃、不持锁，不遗留无限后台任务。
...
| `llm_image_generate(request)` | 文生图代理（§13 首版）：单对象载荷（projectId/jobId/provider 配置/model/prompt/size），key 解密同 `llm_chat`；请求 OpenAI 兼容 `/images/generations`（b64_json 优先，url 成员回退下载），响应体流式限读（主响应 64 MiB 文本 / url 回退 32 MiB 字节，超限即中止）；**作业总预算 600s 自命令进入起算**（[issue #141](https://github.com/hailingu/PlotWeave/issues/141)，已实现）：生成 POST（含响应体读取）受既有 300s 客户端上限约束、必然落在总预算内，url 回退下载链的逐跳 DNS 解析、请求与响应体读取的等待上界均为作业剩余预算，预算耗尽按阶段（解析图像主机/下载图像/读取图像）给出诊断并放弃——产物不写回，逐跳 120s 客户端超时仍作单跳兜底；DNS 解析为阻塞系统调用无法取消，预算耗尽只是放弃等待，解析任务运行至系统解析器返回后其结果被丢弃、不持有应用锁（资源生命周期已文档化）；产物按字节魔数定型 MIME（PNG/JPEG/WebP/GIF，provider 声称的 content-type 不作为依据）、过 32 MiB 上限后经原子写内核落盘进项目 `assets/`，§9.3 预检（形状 + 实路径复验）在命令内、返回前完成，前端单次 IPC 直收已校验的 `source=generated` AssetRef 并入索引。请求返回后与落盘前各查一次取消标志：协作式取消即放弃结果 |
