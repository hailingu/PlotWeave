...
同一项目的会话写入复用保存/删除串行链，主文件使用同目录临时文件、flush、原子 rename 与父目录同步。失败的最新快照保留至成功保存或删除项目；后续消息变更、重开保留会话、再次尝试退出均可重试主文件。正常关闭窗口与 macOS ⌘Q 共用退出屏障：先等在途写入落定，每项目最多尝试一次失败快照；期间新加入的保存也需等待，仍失败则保留窗口并显示错误。没有后台自动重试；强制终止后未保存内容可能丢失，不承诺跨进程恢复。

删除项目等待已有写入并清除项目目录中的会话，成功后清除内存快照；不再有恢复副本清理回执或首页孤儿副本扫描。复制项目仅复制画布和项目资产，不复制对话；现有剧本导出也不包含会话。
...
- **会话生命周期（[issue #47](https://github.com/hailingu/PlotWeave/issues/47)、[PR #57](https://github.com/hailingu/PlotWeave/pull/57)，已实现）**：切换检查器或折叠面板保持会话，重开从主文件恢复。失败会话在项目视图之外保留；加载等待共享保存链后优先采用该进程内快照，重开时提示并尝试保存。成功清除项目级错误与保留内容；读取失败的空回退与损坏诊断不触发挂载保存。只保存主文件、按需重试及退出边界见 §10.1。
