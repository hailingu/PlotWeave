...
- **库文件/索引的可恢复提交协议**：`library/assets/` 文件与 `library.json` 的两个独立操作不构成事务，顺序必须保证失败只产生**孤儿文件**，绝不能产生“索引仍引用、媒体本体已删除”的不可恢复状态；下文的 fsync 均包含目标平台的等价耐久屏障，平台无法提供时命令必须失败，不得继续下一阶段。导入/收藏先按 §7.1 完成目标临时文件的句柄相对写入与文件内容 flush/fsync、原子 rename，并 fsync 最终父目录使落位耐久，确认文件可读后才按上一条耐久提交新增索引；索引提交失败或进程在其前崩溃时文件只是未入索引的孤儿，活动库不出现缺失媒体。删除采用相反顺序：从本次受信索引快照捕获待删条目及已打开文件身份，并保持该文件及最终父目录句柄直到索引提交与后续删除结束；先生成不含该条目的完整索引并完成上一条的文件内容 flush/fsync、原子 rename 与父目录 fsync，只有确认索引已耐久提交后才通过这些受信句柄删除物理文件并 fsync 资产父目录。索引提交失败或其耐久性尚不能确认时物理文件保持原样且命令失败；提交成功后删除或资产目录 fsync 失败、或者进程崩溃时不得把旧索引恢复成指向缺失文件的状态，而是返回/记录 `cleanupPending` 警告，留下或按未确定状态处理孤儿供后续清理。同一文件身份仍被新索引中其他条目引用时只删本条索引、不删物理文件。启动/列表诊断可枚举“资产根普通文件 − 当前规范化索引所引用文件”的孤儿候选，但不得自动删除；显式清理必须在操作当时重新读取索引、重新比较已打开文件身份并再次走 §7.1 句柄链，候选重新被引用时跳过。由此导入、删除任一阶段中断都可恢复，且清理不会误删活动资产。
...
interface CommandPayloads {
  ...
  // relPath 的真实路径包含检查由 Rust 资产命令执行，前端词法检查不能替代。
...
| `save_project(projectId, doc)` | 按 §10.2 验证完整目录、目标与临时文件信任链后，Rust 在命令边界为本次尝试只取一次系统时间，**无条件覆盖**调用方携带的 `doc.project.updatedAt`（不信任旧值、未来值或前端时钟），再以排他创建的同目录临时文件 + flush + rename 原子替换 `project.json`；随后以同一 name/updatedAt 更新可重建的 `index.json` 缓存，跨文件中断由 §10.2 的启动/列表校正恢复。成功回执返回该权威 updatedAt，供前端刷新内存元数据而不触发新一轮脏写；失败重试重新取时，`serializeProject` 只负责结构序列化、不负责保存时刻盖戳 |
...
| `delete_library_asset(assetId)` | 从本次受信规范化索引解析 id，先耐久提交（含 `library/` 目录 fsync）不含该条目的 `library.json`；仅提交成功后，且新索引无其他条目引用同一已打开文件身份时，才按 §7.1 通过受信库资产根句柄执行最终组件不跟随的相对删除并 fsync 资产父目录。后续删除或目录 fsync 失败返回 `cleanupPending`，不得回滚为指向缺失文件的旧索引；不得把缓存 relPath 直接交给 `remove_file`（不影响已拷入项目的副本） |
