...
const EMPTY_SNAPSHOT: AiGraphSnapshot = {
  nodes: [],
  edges: [],
  assets: new Map(),
}
...
describe('AI_NODE_FIELDS（issue 41：节点字段协议单一来源）', () => {
  it('覆盖五类 AI 可写节点；图片节点不在协议内（§13 首版只读）', () => {
    expect(Object.keys(AI_NODE_FIELDS).sort()).toEqual([
      'beat',
      'branch',
      'dialogue',
      'scene',
  ...
  it('字段表与校验白名单同源：按表填满即通过，表外字段被拒', () => {
    for (const [type, data] of Object.entries(VALID_DATA)) {
      ...
      expect(
        createBatch(type, data).ok,
        `${type} 按字段表构造的 data 应通过校验`,
      ).toBe(true)
    }
    const bad = createBatch('beat', {
      ...VALID_DATA.beat,
      label: '立足',
      summary: '小店开张',
    })
