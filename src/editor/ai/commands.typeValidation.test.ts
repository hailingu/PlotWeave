...
describe('issue 49 · 创建与连线类型只接受协议成员', () => {
  it.each(createCases)(
    'create $nodeType，data $shape：以未知类型拒绝',
    ({ nodeType, data }) => {
      const result = validateAiBatch(
        [{ op: 'create_node', nodeType, data }],
        graph(),
      )
...
describe('issue 49 · 共享载荷与修改入口拒绝未知目标类型', () => {
  it.each(prototypeTypes)(
    'payload %s：空/非空载荷均不得把继承属性当白名单',
    (nodeType) => {
...
describe('issue 49 · 批次原子性与纠正后恢复', () => {
  it('混合批次一次收集类型错误，纠正后按原始顺序交付合法命令', () => {
    ...
    expect(corrected.commands.map((cmd) => cmd.op)).toEqual([
      'update_node',
      'create_node',
      'connect_edge',
    ])
  ...
  it.each(['scene', 'beat', 'dialogue', 'branch', 'shot'])(
    '合法 %s 创建与批内 ref 修改仍通过',
    (nodeType) => {
      const data =
        nodeType === 'branch' || nodeType === 'shot'
          ? { prompt: 'x' }
          : { name: 'x' }
      const result = validateAiBatch(
        [
  ...
  it.each([
    ...
    const result = validateAiBatch(
      [{ op: 'connect_edge', ...edge, optionIndex: 0 }],
      richSnap(),
    )
    expect(result.ok).toBe(true)
    expect(result.commands[0]).toMatchObject({
