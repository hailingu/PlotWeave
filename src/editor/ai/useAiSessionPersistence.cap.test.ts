  ...
  return {
    ...
    card: {
      ...
      v: {
        ...
        issues: [],
        items: [
          { kind: 'create', key: `node-${id}`, label: '新场景', danger: false },
        ],
        commands: [
          {
            op: 'create_node',
...
it('多张窗口外待执行卡占用 200 条总容量，并保留最新普通历史', () => {
  ...
  expect(saved.filter((entry) => entry.card)).toEqual([
    proposal(1),
    proposal(2),
  ])
...
it.each([
  ...
  expect(saved.filter((entry) => entry.kind === 'note')).toHaveLength(
    testCase.history,
  )
...
it('较新待执行卡与较旧卡共享容量，输出按原顺序且不重复', () => {
  ...
  expect(saved.filter((entry) => entry.card).map((entry) => entry.id)).toEqual([
    900, 7,
  ])
  expect(new Set(saved.map((entry) => entry.id)).size).toBe(200)
})

it.each(['dismissed', 'executed'] as const)(
  '旧卡转为 %s 后释放优先容量',
  (status) => {
    const thread = [proposal(1), proposal(2), ...notes(3)]
    expect(save(thread)).toHaveLength(200)
    const changed = thread.map((entry) =>
      entry.id === 1 && entry.card
        ? { ...entry, card: { ...entry.card, status } }
        : entry,
    )
...
it('多张未确认执行卡先剔除回执再分配容量，重载保留命令与对账身份', () => {
  ...
  const original = structuredClone(thread)
  const saved = save(thread)
  const restored = normalizeAiSession(
    JSON.parse(JSON.stringify({ schemaVersion: 1, entries: saved })),
  )
  expect(restored.repaired).toBe(false)
  expect(restored.session.entries).toHaveLength(200)
  expect(restored.session.entries.slice(0, 3).map((entry) => entry.id)).toEqual(
    [1, 2, 5],
  )
  expect(restored.session.entries[0].card).toEqual({
    ...proposal(1).card,
    aiRevisionAfter: 8,
  })
  expect(restored.session.entries[1].card).toEqual({
    ...proposal(2).card,
    aiRevisionAfter: 9,
  })
...
it('旧超额会话加载不写回，后续编辑保存并重载后总量收敛', async () => {
  const loaded = normalizeAiSession({
    schemaVersion: 1,
    entries: [proposal(1), proposal(2), ...notes(3)],
  }).session
  ...
  const writeToDisk = async (session: AiSession): Promise<void> => {
    disk = normalizeAiSession(
      JSON.parse(JSON.stringify(diskSessionOf(session))),
    ).session
  }
  const { rerender } = renderHook(
    ({ entries }) =>
      useAiSessionPersistence(entries, null, writeToDisk, undefined),
    { initialProps: { entries: loaded.entries } },
  )
  expect(disk.entries).toHaveLength(202)
  rerender({ entries: [...loaded.entries, ...notes(203, 1)] })
  await waitFor(() => {
    expect(disk.entries).toHaveLength(200)
  })
