...
function check(name: string, payload: object) {
  const parameters = AI_TOOLS.find((tool) => tool.function.name === name)!
    .function.parameters
  const rule: Rule.RuleModule = {
    meta: { schema: [parameters] },
    create: () => ({}),
  }
  return new Linter().verify('', [
    {
      plugins: { contract: { rules: { payload: rule } } },
...
describe('batch 命令逐项参数契约', () => {
  it.each([
    ...
    { op: 'create_node', nodeType: 'dialogue', data: { lines: [{}] } },
    {
      op: 'create_node',
      nodeType: 'dialogue',
      data: { lines: [{ text: '你好', speaker: { name: '店长' } }] },
    },
    {
      op: 'create_node',
      nodeType: 'dialogue',
      data: { lines: [{ kind: 'action', text: '关门', speaker: 'hero' }] },
    },
    ...
    { op: 'create_node', nodeType: 'branch', data: { options: [42] } },
    {
      op: 'create_node',
      nodeType: 'shot',
      data: { refs: [{ kind: 'character', assetId: 'a', label: '两者互斥' }] },
    },
  ...
  it('真实角色→对白→连线批次同时满足供应商 schema 和应用校验', () => {
    ...
    const parsed = toolCallsToCommands([
      {
        id: 'b',
        type: 'function',
        function: {
          name: 'batch',
          arguments: JSON.stringify(batch),
        },
      },
    ])
    expect(
      validateAiBatch(parsed.commands, {
        ...snap(),
...
describe('单工具同样提供完整字段结构', () => {
  ...
  it.each([
    ...
    [
      'create_node',
      { nodeType: 'dialogue', data: { lines: [{ text: '无名角色的台词' }] } },
    ],
    [
      'create_node',
      { nodeType: 'branch', data: { options: ['走', { label: '留' }] } },
    ],
    ['update_node_spec', { nodeId: 'n1', patch: { synopsis: '新的梗概' } }],
    [
      'upsert_character',
      { entityId: 'existing', fields: { bio: '只更新小传' } },
    ],
    ['upsert_document', { fields: { title: '世界观', body: '大陆纪元……' } }],
    [
      'upsert_document',
      { entityId: 'doc-1', fields: { body: '追加世界观设定' } },
    ],
    [
      'upsert_document',
      {
