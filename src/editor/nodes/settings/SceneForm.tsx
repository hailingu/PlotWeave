...
export function SceneForm({
  ...
  readonly node: Extract<PanelNode, { type: 'scene' }>
  ...
  const patch = (p: PatchShape<SceneNodeData>) =>
    patchNode(node.id, { nodeType: 'scene', patch: p })
  ...
  const weather = useCompositionSafeValue(d.weather ?? '', (next) =>
    patch({ weather: next }),
  )
  const synopsis = useCompositionSafeValue(d.synopsis, (next) =>
    patch({ synopsis: next }),
  )
  ...
  return (
    <>
      ...
      <SceneInteriorSegment
        interior={d.interior}
        onPick={(value) => patch({ interior: value })}
      />
      <Field label="梗概">
