...
describe('projectAssets 内存回退：importFromLibrary', () => {
  it('生成项目级 AssetRef：新 id、assets/ 前缀 relPath、规范 mime、source=upload、规范 UTC createdAt', async () => {
    ...
    const lib = await libraryStore.put(
      new File([new Uint8Array([1])], '林晚.PNG', { type: 'image/png' }),
      'character',
    )
  ...
  it('未知库资产拒绝', async () => {
    ...
    await expect(
      projectAssets.importFromLibrary('p-1', 'la-9'),
    ).rejects.toThrow(/库资产不存在/)
  })
})
...
describe('projectAssets 内存回退：revalidate', () => {
  it('无盘上文件可校验：恒通过（重做防线在预览态直通，issue #10）', async () => {
    ...
    await expect(
      projectAssets.revalidate('p-1', asset),
