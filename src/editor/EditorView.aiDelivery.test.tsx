...
async function setup(initial = project()) {
  ...
  return {
    ...
    flow: within(flow),
    outline: within(screen.getByLabelText('故事大纲')),
  }
}
...
describe('明确操作先交付预览，再由用户确认', () => {
  it.each([false, true])(
    '第二节奏卡创建场景 fenced=%s：预览、同步、整批撤销且不重复执行',
    async (fenced) => {
      const h = await setup()
      ipc
        .mockResolvedValueOnce({
          role: 'assistant',
          content: '明白，将在第二个节奏卡后创建场景。',
        })
      ...
      expect(h.outline.getAllByText('场 03 · 吞并计划').length).toBeGreaterThan(
        0,
      )
      await waitFor(() => expect(h.flow.getByText('吞并计划')).toBeTruthy())
      await waitFor(() => expect(h.document()?.nodes).toHaveLength(5))
      const doc = h.document()
      const added = doc.nodes.find(
        (node) => !['n1', 'n2', 'n3', 'n4'].includes(node.id),
      )!
  ...
  it('扩写请求回显批次记录时无预览卡、画布不变且显示未交付诊断（issue 91）', async () => {
    ...
    ipc
      .mockResolvedValueOnce({
        role: 'assistant',
        content: '{"action":true,"query":"修改场04的对白"}',
      })
...
function replyWithExample(
  ...
  const batches = messages
    ...
    .flatMap((message) => [
      ...message.content.matchAll(/```json\n([\s\S]*?)\n```/g),
    ])
    .map(
      (match) =>
        JSON.parse(match[1]) as {
          commands: Array<{ op: string; nodeType?: string }>
        },
    )
  const batch = batches.find((candidate) =>
    candidate.commands.some(
      (command) => command.op === 'create_node' && command.nodeType === type,
    ),
  )
  return batch
    ? {
...
function insertionProject(dialogue: boolean): EditorProjectContent {
  ...
  const context = { against: [], characters: [], center: null }
  initial.nodes.push({
    ...buildCanvasNode('beat', { data: { name: '试探交锋' } }, context),
    id: 'B',
  })
  if (dialogue)
    initial.nodes.push({
...
function links(doc: Pick<ProjectContent, 'edges'>) {
  return doc.edges
    .map((edge) => [edge.source, edge.target, edgeKindOf(edge)].join('→'))
    .sort()
}
...
describe('发送给模型的完整场景插入示例', () => {
  it('替换旧直连、保留无关路径，确认和撤销同步画布与大纲', async () => {
    ...
    ipc.mockImplementation(async (_command, args) =>
      replyWithExample((args as { messages: ChatMessage[] }).messages, 'scene'),
    )
    ...
    const added = h
      .document()
      .nodes.find((node) => !initial.nodes.some((old) => old.id === node.id))!
...
describe('发送给模型的完整角色与对白创建示例', () => {
  it('场03后插入对白，speaker 解析为新增角色，实体和剧情流同批撤销', async () => {
    ...
    ipc.mockImplementation(async (_command, args) =>
      replyWithExample(
        (args as { messages: ChatMessage[] }).messages,
        'dialogue',
      ),
    )
