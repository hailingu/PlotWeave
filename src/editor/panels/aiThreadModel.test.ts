        ...
        commands: [
          {
            op: 'update_node',
            nodeId: 'd1',
            patch: { nodeType: 'dialogue', patch: { lines: [] } },
          },
        ],
        items: [
          {
...
describe('buildMessages 批次执行上下文（issue 73）', () => {
  it('同一回复的待确认与执行成功产生不同请求，成功记录绑定原消息', () => {
    ...
    expect(
      JSON.parse(historyOf(executed)[0].content.split('\n').slice(-1)[0]),
    ).toMatchObject({
      batchId: 2,
      status: 'executed',
      commandCount: 1,
...
describe('buildMessages 批次状态边界', () => {
  it.each(['pending', 'dismissed'] as const)(
    '%s 卡不可被表述为执行成功',
    (status) => {
      expect(recordOf(batchEntry(status))).toMatchObject({
        batchId: 2,
        status,
        commandCount: 1,
      })
  ...
  it('校验失败保留诊断，不把已解析的命令子集视为待执行的合法整批', () => {
    ...
    expect(recordOf(entry)).toMatchObject({
  ...
  it('大批次只保留有限预览，完整命令与长台词不重复进入历史', () => {
    ...
    entry.card!.v.commands = Array.from({ length: 100 }, () => ({
      ...
      data: {
        name: '新节点',
        lines: [{ kind: 'action', text: '不应重复的台词正文'.repeat(10_000) }],
      },
    }))
    entry.card!.v.items = Array.from({ length: 100 }, (_, i) => ({
      kind: 'create',
      key: `c${i}`,
      label: `新建${i}：${'长名称'.repeat(1000)}`,
      danger: false,
    }))
  ...
  it('字符预算包含批次记录，条数裁剪不会留下旧批次的孤立回执', () => {
    ...
    const history = historyOf(
      buildMessages([msg(1, 'user', 'x'.repeat(47_900)), entry], '继续', false),
    )
    expect(history).toHaveLength(2)
    const thread = [
      entry,
      ...Array.from({ length: 40 }, (_, i) => msg(i + 3, 'user', `m${i}`)),
      {
        id: 44,
        kind: 'note' as const,
        text: '✓ 已执行 1 项改动。',
    ...
    expect(
      historyOf(buildMessages(thread, '继续', false)).every(
        (m) => !m.content.includes('batchId'),
      ),
    ).toBe(true)
  })
})
...
describe('buildMessages 批次归属与旧历史兼容', () => {
  it('交错追加的回执不覆盖另一个批次的状态，也不由旧文本推测执行结果', () => {
    ...
    const records = historyOf(buildMessages(thread, '继续', false))
      .slice(0, -1)
      .map(
        (m) =>
          JSON.parse(m.content.split('\n').slice(-1)[0]) as Record<
            string,
            unknown
          >,
      )
    expect(records).toMatchObject([
      { batchId: 2, status: 'executed' },
      { batchId: 4, status: 'pending' },
    ])
  })
...
describe('buildMessages 历史上界', () => {
  it('条数超界时自旧侧截断，仅保留最新一段历史', () => {
    const thread = Array.from({ length: 50 }, (_, i) =>
      msg(i + 1, i % 2 ? 'assistant' : 'user', `m${i}`),
    )
...
describe('readToolOf · 读工具分发（issue 56 增 get_document）', () => {
  ...
  it('get_document 按 id 分发到文档读取器；缺省读取器返回 not found', () => {
    const readDocument = (id: string) =>
      id === 'doc-1'
        ? JSON.stringify({ id: 'doc-1', title: '世界观', body: '全文' })
        : null
    const tool = readToolOf(undefined, undefined, () => '{}', readDocument)
    expect(tool('get_document', args({ documentId: 'doc-1' }))).toContain(
      '世界观',
    )
    expect(tool('get_document', args({ documentId: 'ghost' }))).toBe(
      'document not found: ghost',
    )
  })
