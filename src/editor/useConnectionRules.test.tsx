...
describe('useConnectionRules（§4.3/§4.4 连线规则）', () => {
  ...
  it('放行合法连线：sequence / 分支选项出口 / 无宿主 attach', () => {
    const { result } = setup()
    expect(result.current.rules.isValidConnection(conn('sc2', 'sc3'))).toBe(
      true,
    )
    expect(
      result.current.rules.isValidConnection(conn('br1', 'sc1', 'option-o1')),
    ).toBe(true)
    expect(
      result.current.rules.isValidConnection(conn('sc3', 'sh2', 'shots')),
    ).toBe(true)
  })
  ...
  it('onConnect 按端口建成 sequence 边并单步入栈可撤销', () => {
    ...
    const added = result.current.doc.edges[
      result.current.doc.edges.length - 1
    ] as Edge
    ...
    act(() => commands[0].undo())
    expect(result.current.doc.edges.map((e) => e.id)).not.toContain(
      'e-sc2-out-sc3',
    )
    act(() => commands[0].redo())
  ...
  it('onConnect 按端口区分 branch 边与 attach 边', () => {
    ...
    expect(
      result.current.doc.edges[result.current.doc.edges.length - 1],
    ).toMatchObject({
      type: 'branch',
    })
