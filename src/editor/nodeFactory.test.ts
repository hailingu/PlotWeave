...
describe('buildCanvasNode（节点工厂：默认字段 + 落点）', () => {
  ...
  it('对白：首行说话人取设定集首位角色（带稳定 id）；无角色时 speaker 为 undefined', () => {
    ...
    const lines = withChar.data.lines as Array<{
      id: string
      kind: string
      speaker?: string
    }>
  ...
  it('节奏卡 / 分支 / 分镜卡默认字段；分镜镜号独立编号', () => {
    ...
    const shot = buildCanvasNode(
      'shot',
      undefined,
      ctx({ against: [shotNodeOf('sh1', 3)] }),
    )
  ...
  it('落点：opts.at 优先；否则视口中心 + 节点数阶梯偏移；无中心则原点', () => {
    ...
    const c = buildCanvasNode(
      'beat',
      undefined,
      ctx({ center: { x: 500, y: 300 } }),
    )
    expect(c.position).toEqual({ x: 330, y: 240 })

    // 基线 3 个节点 → cascade = (3 % 5) * 28 = 84
    const against: CanvasNode[] = [
      sceneNode('a', 1),
      sceneNode('b', 2),
      sceneNode('c', 3),
    ]
    const cascaded = buildCanvasNode(
      'beat',
      undefined,
      ctx({ against, center: { x: 500, y: 300 } }),
    )
  ...
  it('批量基线：ctx.against 传模拟数组时编号跟随虚拟终态（防重号）', () => {
    ...
    const first = buildCanvasNode(
      'scene',
      undefined,
      ctx({ against: simNodes }),
    )
    expect(first.data.sceneNo).toBe(2)
    const second = buildCanvasNode(
      'scene',
      undefined,
      ctx({ against: [...simNodes, first] }),
    )
...
describe('图片节点工厂（§13 文生图首版默认字段）', () => {
  it('默认 prompt 空、model 未选、size 竖版短剧画幅（plan.ts 声明的默认档）、outputs 空容器', () => {
  ...
  it('opts.data 覆盖默认字段（如从画布拖入预填 prompt）', () => {
    ...
    expect(node.data.size).toBe('1024x1536')
  })
})
