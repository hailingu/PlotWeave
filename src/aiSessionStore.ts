...
export async function loadAiSession(id: string): Promise<AiSession> {
  if (!isTauri) return memorySessions.get(id) ?? { schemaVersion: 1, entries: [] }
  const { invoke } = await import('@tauri-apps/api/core')
  const { session, repaired } = normalizeAiSession(await invoke<unknown>('load_ai_session', { id }))
  if (repaired) void saveAiSession(id, session).catch((err: unknown) => console.warn('[aiSession] 修复会话回写失败', err))
  return session
}
