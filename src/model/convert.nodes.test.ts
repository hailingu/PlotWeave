...
describe('归一化：对白行判别与必填字段（§4.2 DialogueLine，§11.3）', () => {
  it('kind 非法或 text 非字符串的行隔离并警告，合法行保留——坏行不阻挡画布打开', () => {
    ...
    expect(
      round.warnings.some((w) => w.includes('d1') && w.includes('spec.lines')),
    ).toBe(true)
  ...
  it('可选字段异型（speaker 对象 / side 非左右值 / vo 非布尔）：逐字段剥离并警告，行与合法字段保留', () => {
    ...
    const round = parseProject(doc)
    const lines = (
      round.content.nodes.find((n) => n.id === 'd1')!.data as unknown as {
    ...
    for (const f of ['speaker', 'side', 'vo']) {
      expect(
        round.warnings.some(
          (w) => w.includes('d1') && w.includes('l1') && w.includes(f),
        ),
      ).toBe(true)
    }
  })
...
describe('归一化：节点判别联合形状校验（§4.1/§11.1 第 3 步，§9.3 create_node 的加载侧对等）', () => {
  const specOf = (doc: ProjectDocument, id: string) =>
    (
      doc.graph.nodes.find((n) => n.id === id)!.data as unknown as {
  ...
  it('spec 必填标量异型（beat.tone 为对象）：节点隔离并警告——渲染 React 子节点的崩溃源', () => {
    ...
    expect(
      round.warnings.some((w) => w.includes('b1') && w.includes('tone')),
    ).toBe(true)
  })
  ...
  it('spec 必填标量缺失（synopsis/prompt/size）与未知节点类型：隔离并警告，关联边随孤儿规则隔离', () => {
    ...
    ;(doc.graph.nodes.find((n) => n.id === 'd1')! as { type: string }).type =
      'montage'
    const round = parseProject(doc)
    ...
    expect(
      round.warnings.some((w) => w.includes('s1') && w.includes('synopsis')),
    ).toBe(true)
    expect(
      round.warnings.some((w) => w.includes('br1') && w.includes('prompt')),
    ).toBe(true)
    expect(
      round.warnings.some((w) => w.includes('sh1') && w.includes('size')),
    ).toBe(true)
    expect(
      round.warnings.some((w) => w.includes('d1') && w.includes('类型')),
    ).toBe(true)
  })
  ...
  it('名称型节点缺必填 meta.label：隔离并警告（§4.1 LabeledMeta）', () => {
    ...
    expect(round.content.nodes.map((n) => n.id)).not.toContain('b1')
    expect(
      round.warnings.some((w) => w.includes('b1') && w.includes('label')),
    ).toBe(true)
  })
  ...
  it('episodeNo 非法（零/负/小数/超安全整数）：删除字段回退未分集并警告，节点保留', () => {
    ...
    for (const id of ['s1', 'b1', 'd1', 'br1']) {
      expect(
        'episodeNo' in round.content.nodes.find((n) => n.id === id)!.data,
      ).toBe(false)
    }
    expect(round.warnings.filter((w) => w.includes('episodeNo'))).toHaveLength(
      4,
    )
  })
  ...
  it('非法 sceneNo/shotNo：按文档序顺位重发为正整数并警告（场号/镜号可在面板修正）', () => {
    ...
    const shot = round.content.nodes.find((n) => n.id === 'sh1')!.data as {
      shotNo: number
    }
    expect(scene.sceneNo).toBe(1)
    expect(shot.shotNo).toBe(1)
    expect(
      round.warnings.some((w) => w.includes('s1') && w.includes('sceneNo')),
    ).toBe(true)
    expect(
      round.warnings.some((w) => w.includes('sh1') && w.includes('shotNo')),
    ).toBe(true)
  })
})
...
describe('归一化：键控列表 id 非空且数组内唯一（§4.2/§11.1 第 3 步）——branch 选项', () => {
  it('branch 选项重复 id：保留首见项、后续重发新 id；绑定既有选项的连线不改接', () => {
    const doc = serializeProject(
      ...
      'p-1',
      NOW,
    ) as unknown as RawOptionsDoc
    optionsOf(doc, 'br1').push({ id: 'opt-1', label: '重复项' })
    const round = parseProject(doc)
    const options = (
      round.content.nodes.find((n) => n.id === 'br1')!.data as {
    ...
    // e2 绑定 opt-2 不受影响
    expect(round.content.edges.find((e) => e.id === 'e2')!.sourceHandle).toBe(
      'option-opt-2',
    )
    expect(
      round.warnings.some((w) => w.includes('br1') && w.includes('重复')),
    ).toBe(true)
  })
  ...
  it('唯一空选项 id：重发新 id 并同步改写该 branch 引出边的 option- 句柄（连线保留）', () => {
    ...
    expect(
      round.warnings.some((w) => w.includes('e-empty') && w.includes('改写')),
    ).toBe(true)
  })
  ...
  it('同 branch 多个空选项 id：映射歧义不改写，指向空句柄的连线按孤儿边隔离', () => {
    ...
    const options = (
      round.content.nodes.find((n) => n.id === 'br2')!.data as {
        options: { id: string }[]
      }
    ).options
...
describe('归一化：键控列表 id 非空且数组内唯一（§4.2/§11.1 第 3 步）——dialogue lines / shot refs', () => {
  it('dialogue lines / shot refs 的重复与空 id：重发去歧（不被边引用，纯列表 key）', () => {
    ...
    const spec = (id: string) =>
      doc.graph.nodes.find((n) => n.id === id)!.data.spec
    spec('d1').lines!.push(
      { id: 'line-1', kind: 'line', text: '重复 id' },
      { kind: 'action', text: '无 id' },
    )
    spec('sh1').refs!.push(
    ...
    expect(
      round.warnings.filter((w) => w.includes('重发')).length,
...
describe('归一化：非法节点 id 重发与空端点改写（§8.1/§11.1 第 3 步）', () => {
  ...
  it('唯一空 id 节点：建立「空 id → 新 id」映射，指向空串的边端点同步改写、连线保留', () => {
    ...
    expect(
      round.warnings.some((w) => w.includes('e-b') && w.includes('改写')),
    ).toBe(true)
  })
...
describe('归一化：键控列表成员的类型字段校验（§4.2 BranchOption/ShotRef 完整判别联合，§9.3 加载侧对等）', () => {
  it('选项 label 非字符串/缺失、引用位 kind 未知/label 异型/assetId 与 label 两落或两缺：成员移除并警告，合法成员保留', () => {
    ...
    const shData = round.content.nodes.find((n) => n.id === 'sh1')!.data as {
      refs: unknown[]
    }
...
describe('归一化：场景 time/weather 文本字段校验（§11.1 节点校验细则，SceneNode 渲染安全）', () => {
  ...
  it('场景 time 缺失（§4.2 可选缺省）：确定性置空串并警告，节点保留', () => {
    const missing = serializeProject(
      mkContent(),
      'p-1',
      NOW,
    ...
    expect(
      round.warnings.some(
        (w) => w.includes('s1') && w.includes('time') && w.includes('置空串'),
      ),
    ).toBe(true)
  })

  it('场景 time 非字符串（形态错位，如 time: {}）：节点隔离并警告——对象进会话会被 SceneNode 当 React 子节点渲染', () => {
    const badType = serializeProject(
      mkContent(),
      'p-1',
      NOW,
    ) as unknown as SceneDoc
    ...
    expect(
      round.warnings.some(
        (w) => w.includes('s1') && w.includes('time') && w.includes('隔离'),
      ),
    ).toBe(true)
  })
  ...
  it('场景 weather 非字符串：剥离该可选字段并警告，节点保留', () => {
    ...
    expect(
      round.warnings.some(
        (w) => w.includes('s1') && w.includes('weather') && w.includes('剥离'),
      ),
    ).toBe(true)
  })
})
...
describe('§11.1 第 3 步顺序：身份修复先于形状隔离（重复 id 不改接语义）', () => {
  ...
  it('重复节点 id：首见（异型被隔离）占据 id，后见节点重发新 id，指向原 id 的边按孤儿隔离', () => {
    const doc = serializeProject(mkContent(), 'p-1', NOW)
    ;(doc.graph.nodes as unknown as Record<string, unknown>[]).splice(
      0,
      doc.graph.nodes.length,
      {
        id: 's0',
        type: 'scene',
        layout: { position: { x: 0, y: 0 } },
        data: { spec: sceneSpec(), meta: { label: '源头' } },
      },
      // 首见 n1：spec 缺 synopsis（REQUIRED_SCALARS）→ 形状隔离
      {
        id: 'n1',
        type: 'scene',
        layout: { position: { x: 1, y: 0 } },
        data: {
          spec: { sceneNo: 2, interior: true, time: '', characterIds: [] },
          meta: { label: '坏节点' },
        },
      },
      // 后见 n1：合法
      {
        id: 'n1',
        type: 'scene',
        layout: { position: { x: 2, y: 0 } },
        data: { spec: sceneSpec({ sceneNo: 3 }), meta: { label: '好节点' } },
      },
    )
    ;(doc.graph.edges as unknown as unknown[]).splice(
      0,
    ...
    const round = parseProject(doc)
    // 后见节点持新 id（不再是 n1）
    const labels = round.content.nodes.map(
      (n) => (n.data as { name?: string }).name,
    )
    expect(labels).toContain('源头')
    expect(labels).toContain('好节点')
    const good = round.content.nodes.find(
      (n) => (n.data as { name?: string }).name === '好节点',
    )!
  ...
  it('重复选项 id：首见（异型被过滤）占据 id，后见选项重发新 id，option-x 连线按孤儿隔离', () => {
    ...
    ;(doc.graph.nodes as unknown as Record<string, unknown>[]).splice(
      ...
      doc.graph.nodes.length,
      {
        ...
        type: 'scene',
        layout: { position: { x: 0, y: 0 } },
        data: { spec: sceneSpec(), meta: { label: '靶' } },
      },
      {
        id: 'br1',
        type: 'branch',
