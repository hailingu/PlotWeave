...
describe('边句柄规范（§5：匿名端口句柄必须省略）', () => {
  ...
  it('加载剥离：已知 kind 边携带 targetHandle 不隔离，剥离并警告', () => {
    ...
    expect(round.content.edges.every((e) => e.targetHandle === undefined)).toBe(
      true,
    )
  ...
  it('data.order 异型（非有限数）：确定性剥离并警告，边本身保留不隔离（§5 存在时须为有限数）', () => {
    ...
    expect(round.content.edges.map((e) => e.id)).toEqual(['e1', 'e2', 'e3'])
    expect(
      round.content.edges.every(
        (e) => (e.data as { order?: unknown } | undefined)?.order === undefined,
      ),
    ).toBe(true)
...
describe('归一化：§11.1 第 3 步 id 重发与成环隔离——重复节点 id / 自环 / 剧情流成环', () => {
  it('重复节点 id：保留文档序首个，后续重发本域未占用新 id（按 id 的引用仍解析到首见节点）', () => {
    ...
    const orphan = round.content.nodes[5].data as {
      name?: string
      lines?: unknown[]
    }
    ...
    expect(
      round.warnings.some((w) => w.includes('重复') && w.includes('d1')),
    ).toBe(true)
  })
  ...
  it('自环边（source === target）隔离并警告', () => {
    ...
    expect(
      round.warnings.some((w) => w.includes('e-self') && w.includes('自环')),
    ).toBe(true)
  })
  ...
  it('成环边：按文档序重建剧情流图，加入即闭合回路的 sequence 边隔离', () => {
    ...
    expect(
      round.warnings.some((w) => w.includes('e-back') && w.includes('环')),
    ).toBe(true)
  })
  ...
  it('成环边：branch 边同样参与环检测（传递闭包）', () => {
    ...
    expect(
      round.warnings.some((w) => w.includes('e-cycle') && w.includes('环')),
    ).toBe(true)
  })
})
...
describe('归一化：§11.1 第 3 步——attach 宿主唯一、端口归属与逻辑重复边', () => {
  ...
  it('同一 shot 的多条入向 attach 边：保留文档序首条，其余按孤儿边隔离并警告', () => {
    const doc = serializeProject(mkContent(), 'p-1', NOW)
    const secondScene = structuredClone(
      doc.graph.nodes.find((n) => n.id === 's1')!,
    )
    secondScene.id = 's2'
    ...
    expect(
      round.warnings.some((w) => w.includes('e-attach2') && w.includes('宿主')),
    ).toBe(true)
  })
  ...
  it('sequence 边 source 为 branch 节点：按孤儿边隔离并警告（§5 端口归属反向约束）', () => {
    ...
    const round = parseProject(doc)
    expect(round.content.edges.map((e) => e.id)).not.toContain(
      'e-seq-from-branch',
    )
    expect(round.warnings.some((w) => w.includes('e-seq-from-branch'))).toBe(
      true,
    )
  ...
  it('同 source/target/sourceHandle 的逻辑重复边：保留文档序首条，其余隔离并警告（§11.3）', () => {
    ...
    expect(
      round.warnings.some((w) => w.includes('e-dup-seq') && w.includes('重复')),
    ).toBe(true)
    expect(
      round.warnings.some((w) => w.includes('e-dup-opt') && w.includes('重复')),
    ).toBe(true)
  })
})
...
describe('归一化：§11.1 第 3 步——重复/空白边 id 与 repaired 回写标志', () => {
  it('重复边 id：保留文档序首条原 id，后续同 id 边重发新 id 并警告（§11.1 第 3 步）', () => {
    ...
    expect(
      round.warnings.some((w) => w.includes('重复') && w.includes('e1')),
    ).toBe(true)
  })
  ...
  it('空白边 id：非空串同款按 §8.1 trim 口径重发新 id 并警告', () => {
    ...
    const round = parseProject(doc)
    const reissued = round.content.edges.find(
      (e) => e.source === 'b1' && e.target === 'd1',
    )!
    expect(reissued.id).toMatch(/^edge-/)
    expect(round.warnings.some((w) => w.includes('边 id 缺失或非法'))).toBe(
      true,
    )
  })
...
describe('归一化：边句柄的运行时类型校验（§11.1，JSON 边界类型擦除）', () => {
  ...
  it('branch 边 sourceHandle 非字符串：隔离该边并警告，其余边保留、项目照常打开', () => {
    ...
    expect(
      round.warnings.some(
        (w) => w.includes('e2') && w.includes('sourceHandle'),
      ),
    ).toBe(true)
  ...
  it('attach 边 sourceHandle 为对象：隔离该边并警告，不触发 startsWith 类型异常', () => {
    const doc = serializeProject(mkContent(), 'p-1', NOW) as unknown as EdgeDoc
    doc.graph.edges.find((e) => e.id === 'e3')!.sourceHandle = {
      handle: 'shots',
    }
    const round = parseProject(doc)
    ...
    expect(
      round.warnings.some(
        (w) => w.includes('e3') && w.includes('sourceHandle'),
      ),
    ).toBe(true)
