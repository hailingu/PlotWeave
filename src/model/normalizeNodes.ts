...
function stripIllegalSpeaker(
  ...
  warnings.push(
    `节点 ${nid} 的对白行 ${lid} 的 speaker ${action ? '为 action 行携带（只允许 line 行有说话人）' : '异型（须为角色 id 字符串）'}，已剥离`,
  )
  delete line.speaker
}
...
function normalizeDialogueLineOptionals(
  ...
  for (const line of spec.lines as Record<string, unknown>[]) {
    ...
    if (
      ...
      warnings.push(
        `节点 ${nid} 的对白行 ${lid} 的 side 异型（须为 left/right），已剥离`,
      )
      delete line.side
    }
    if (line.vo !== undefined && typeof line.vo !== 'boolean') {
      warnings.push(
        `节点 ${nid} 的对白行 ${lid} 的 vo 异型（须为布尔），已剥离`,
      )
      delete line.vo
    }
  }
...
function stripForbiddenMeta(
  ...
  if (
    (type === 'branch' || type === 'shot' || type === 'image') &&
    'label' in meta
  ) {
  ...
  if ((type === 'shot' || type === 'image') && 'episodeNo' in meta) {
    warnings.push(
      `节点 ${nid} 携带 never 禁写的 meta.episodeNo（不参与大纲分组），已剥离`,
    )
...
function normalizeSceneTextFields(
  ...
  if (typeof spec.time !== 'string')
    return 'spec.time 类型错误（spec 形态错位）'
  return null
}
...
function nodeDiscriminantError(
  ...
  for (const [field, kind] of Object.entries(REQUIRED_SCALARS[type] ?? {})) {
    if (typeof data.spec[field] !== kind)
      return `spec.${field} 缺失或类型错误（spec 形态错位）`
  }
  return null
}
...
function normalizeLayoutOptionals(
  ...
  if ('size' in layout) {
    ...
    if (!ok) {
      warnings.push(
        `节点 ${nid} 的 layout.size 非法（须为普通对象且 width/height 为正有限数），已剥离`,
      )
      delete layout.size
    }
  }
  if (
    'zIndex' in layout &&
    (typeof layout.zIndex !== 'number' || !Number.isFinite(layout.zIndex))
  ) {
...
export function normalizeNode(
  ...
  const nid =
    typeof member.id === 'string' && member.id ? member.id : '(缺失 id)'
  ...
  if (
    ...
    warnings.push(
      `节点 ${nid} 的 data/spec/meta/layout 容器缺失或异型，无法机械修复，已隔离`,
    )
    return null
  }
  ...
  if (
    ...
    warnings.push(
      `节点 ${nid} 的 layout.position 坐标非法，无法机械修复，已隔离`,
    )
    return null
  }
  ...
  if (member.type === 'dialogue')
    normalizeDialogueLineOptionals(data.spec, nid, warnings)
...
export function renumberSeqFields(
  nodes: StoryNode[],
  warnings: string[],
): void {
  const used: Record<'scene' | 'shot', Set<number>> = {
    scene: new Set(),
    shot: new Set(),
  }
  for (const n of nodes) {
    ...
    spec[key] = next
    warnings.push(
      `节点 ${n.id} 的 spec.${key} 非法（须为正的安全整数），已按文档序顺位重发为 ${next}`,
    )
  }
}
