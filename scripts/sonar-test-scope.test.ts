...
describe('vite 覆盖率排除与设施清单同源（issue #343）', () => {
  ...
  it('coverage.exclude 反向核验：不命中清单外的维护模块', () => {
    ...
    // 字符）：brace 展开、字符类、extglob 与否定模式按 Vitest 实际语义
    // 与转换器分叉，会让反向核验静默失效——显式拒绝而非误通过
    // （issue #343）。拒绝后剩余语法在两个引擎中均为字面或同义。
    for (const pattern of exclude) {
      ...
      expect(
        unsupported,
        `coverage.exclude 使用了本守卫不支持的模式（brace/字符类/extglob/否定），须改用简单 glob 或扩展匹配器：${pattern}`,
      ).toBe(false)
    }
